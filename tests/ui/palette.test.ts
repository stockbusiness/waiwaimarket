 ...
 * 鮮やかに見えるが、白地に置くと 3.33 / 1.65 / 3.00 しかなく、文字として
 * 読めない。数字で止めておかないと、次に触る人が「ブランドカラーだから」と
 * ボタンやリンクに使ってしまう。
 *
 * 2026-09-19 にロゴの SVG（正式データ）の値へ差し替えた。4 色とも
 * わずかに動いたが、朱・黄・ティールが装飾専用である結論は変わらない。
 *
...
describe("配色のコントラスト", () => {
  ...
  it("装飾色の上に文字を置くなら紺。ただし地色は黄に限る", () => {
    // バッジなどでやむを得ず文字を載せる場合の逃げ道。
    // **AA を満たすのは黄の上だけ**（紺を載せて 6.50）。
    expect(contrast(token("--accent"), token("--brand-amber"))).toBeGreaterThanOrEqual(AA);

    // ティール 3.58 / 朱 3.22。どちらも足りないので、地色として選ばせない。
    // 実データへ更新する前はティールが 4.01 で、やはり足りていなかった
    for (const name of ["--brand-teal", "--brand-coral"]) {
      expect(contrast(token("--accent"), token(name))).toBeLessThan(AA);
    }
  });
