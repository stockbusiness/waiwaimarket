...
注文番号（`order_number`）は `WM-20260919-K7P2NX` の形（2026-09-19 決定）。連番にしないのは、購入者が自分の番号を見ただけで累計注文数を推測できてしまうため。日付は JST で入れる（問い合わせでの照合に使う）。ランダム部は暗号用の乱数から 6 桁、`0`/`O`・`1`/`I`/`L` を除いた 31 文字を棄却法で均等に選ぶ（`lib/orders/number.ts`）。一意性は `order_number` の一意制約が最後の砦で、衝突したらアプリが引き直す。

注文の作成と金額の確定はサーバー処理（service_role）だけが行う。0015 は `orders` に insert のポリシーを置いていない。テナントは自店の注文の**状態だけ**動かせ、金額・配送先・ポイントルールの変更は `orders_guard_columns()` が拒否する。購入者は**決済前の自分の注文を取り消す**ことだけできる（`orders_buyer_cancel` が `using` で `pending`、`with check` で `cancelled` に絞る）。

• order_items：注文明細。商品名と単価は**注文時点の値を写し取る**（商品が後から改名・値下げされても、何をいくらで買ったかは変わってはいけない）

• payments：決済状態、Stripe PaymentIntent ID

• shipments：配送・追跡番号・発送登録日。`shipped_at` はポイント確定（発送登録日＋14日）の起点なので、状態を `shipped` にしたのに行が無い、という状態を作らない。状態の更新と記録の挿入は 0015 の `ship_order()` が 1 回でまとめて書く
