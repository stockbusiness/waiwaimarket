...
- ドメイン（未取得。正式名称は決定済み）
- ロゴ画像の正式データ（背景が透明な SVG。届くまでヘッダーは文字ロゴ、favicon は無し）
...
### ブランド配色（2026-09-18 決定）

ロゴ（waiwaimarket）から実測した 4 色を使う。値は `app/globals.css`。

| | hex | 白地での比 | 用途 |
|---|---|---|---|
| 紺 | `#0B3064` | 12.95 | CTA・フッター（白文字） |
| リンク | `#12539E` | 7.62 | 紺の家系を明るくした色 |
| 朱 | `#F8503C` | 3.38 | **装飾のみ** |
| 黄 | `#F8B824` | 1.77 | **装飾のみ** |
| ティール | `#0CA098` | 3.23 | **装飾のみ** |

**文字を載せられるのは紺だけ。** 朱・黄・ティールは白地で AA（4.5:1）に
届かず、その色を背景にして白文字を載せても比は同じなので、やはり読めない。
文字色・リンク・ボタンに使わないこと。バッジなどでやむを得ず文字を載せる
なら紺を載せる（黄の上で 7.32、ティールの上で 4.01、朱の上で 3.83）。
`tests/ui/palette.test.ts` が比を計算して固定してある（リンクを朱にする、
フッターをティールにする、といった変更で実際に落ちることを確認済み）。

ブランド色が画面から消えないよう、ヘッダーの下に幌のストライプを 4px
敷いてある（`components/ui/site-header.tsx`）。並び順はロゴを実測して
朱→黄→ティール→黄にしてある。
