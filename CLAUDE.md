...
決めた（2026-09-12）。**送料は内訳に含めない。** 送料の税率が docs で
未定義のため。領収書を作るフェーズ3-8 までに決める必要がある。
...
**地域別送料（`shipping_profiles.region_rules`）は使わない。**
jsonb の構造が docs で未定義。基本送料と送料無料しきい値だけを使う。
