...
- **本部管理者の操作には多要素認証（AAL2）が要る。** MFA の登録画面が未実装のため、
  テナントの**停止・停止解除は現時点では行えない**。審査・承認・差戻しは
  本部オペレーター権限なので利用できる。MFA は PR C で実装する
- 商品画像・本人確認ファイルの Storage Policy が未設定（PR C）
- 店舗ページと特商法表示が未実装（PR C）
