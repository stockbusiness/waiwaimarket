...
2. マイグレーションを適用する
   ...
   `0001_init.sql` から `0010_product_management.sql` までが順に流れる。
...
- 公開の商品一覧・商品詳細は未実装。フェーズ2-4 で作る（docs/06）
- 在庫引当（TTL 15 分）は未実装。`inventory_reservations` のテーブルはあるが、
  引当・解放の処理が無い。フェーズ2-3
