...
2. マイグレーションを適用する
   ...
   `0001_init.sql` から `0016_point_rules_seed.sql` までが順に流れる。
   ...
   **下書きとして**作る。雛形のまま公開すると誤った内容を掲示することに
   ...
   `0013` は `orders.shipping_address` に検査制約を付ける。**既存行を書き換えない。**
   形の合わない注文があれば移行はそこで止まる。注文の配送先を黙って別の値に
   すり替えるほうが、移行が失敗するより悪い。止まった場合はその注文を個別に
   確認すること（注文は未実装なので現状は 0 件）。
   ...
   **`0015` から引当解放バッチの役割が増える。** 期限切れの引当を解放した
   ...
   `0016` はポイントの初期ルール（還元 1%・利用上限 50%・確定 14 日・
   期限 12 か月）と負担元（本部）を入れる。`on conflict do nothing` なので
   二度流しても増えない。**これが無いと、決済を繋いでもポイントは
   付与されない。** 入っているかは `/admin/points` で確認できる
   （「基本還元のルールが設定されていません」と出たら未適用）。

   `0016` は「閉じていない基本ルールは 1 本だけ」という部分一意索引を作る。
   ルールの変更は**前の版を閉じてから新しい版を足す**順序でなければ索引に
   弾かれる（`/admin/points` の保存はその順で行う）。既存の版は上書きせず
   残すので、過去の注文の還元率は変わらない。
...
3. スキーマを検証する
   ...
   **psql が使える場合**は、ロールを切り替えて実挙動まで確かめる版がある。
   ...
   他人の注文・配送先・問い合わせ・ポイント残高が読めない、
   本部オペレーターは還元率を変えられない）
   を 91 項目の判定表で返す。検証用の行は最後に削除する。
...
- **購入手続き（決済）は未実装。** カートまでは動くが、`checkout/preview` と
  注文の作成が無いため、カート画面に購入手続きの導線は出ない（フェーズ3）
- 消費税の税率別内訳は**切り捨て**で計算する（2026-09-12 に決定）。
  送料は内訳に含めていない。送料の税率が docs で未定義のため、領収書を作る
  フェーズ3-8 までに決める必要がある
- 地域別送料（`shipping_profiles.region_rules`）は未対応。jsonb の構造が
  docs で未定義のため、基本送料と送料無料しきい値だけを使う
- 商品名の検索は部分一致（`ilike`）で、全文検索の索引を入れていない。
  docs/06 フェーズ6 の想定が商品 20〜50 点のため。件数が増えたら索引を足す
- **ポイントは「設定と表示」までで、付与はまだ走らない。** 還元ルールは
  `/admin/points` で変更でき、購入者は `/points` で残高と履歴を見られるが、
  付与の起点は決済成功（docs/02 6.3）なので、Stripe を繋ぐまで数字は 0 のまま。
  台帳への書き込みと確定・失効バッチは決済と同時に入れる
- 決済・精算は未実装。フェーズ3 以降
